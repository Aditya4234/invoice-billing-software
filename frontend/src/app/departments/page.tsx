...
import { getDepartments, getAllEmployees, getApiData } from "@/lib/api";
...
export default function DepartmentsPage() {
  ...
  useEffect(() => {
    getApiData(() => getDepartments(), []).then((res) => {
      setDepartments(res.map((d: any) => ({
        ...d,
        headName: d.head,
      })));
    });
    getApiData(() => getAllEmployees(), []).then((res) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 lg:p-8 min-w-0">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => showToast(`Editing ${dept.name} department`)} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 transition">
                  <Edit size={14} />
                </button>
              </div>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add New Department" size="md">
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-gray-700">Department Name</label>
            <input type="text" placeholder="Enter department name" className="h-10 w-full rounded-xl border border-gray-200 bg-white px-3.5 text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <input type="number" placeholder="0" className="h-10 w-full rounded-xl border border-gray-200 bg-white px-3.5 text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100" />
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <button onClick={() => { setShowAdd(false); showToast("Department added successfully"); }} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 transition">Save Department</button>
          </div>
        </div>
      </Modal>
