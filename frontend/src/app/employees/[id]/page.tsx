...
export default function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const b = await getApiData(() => getEmployee(id), null);
      if (b) {
        ...
        // Fetch related data
        const att = await getApiData(() => getAttendance(mappedEmp.employeeId).then(r => r.data), []);
        setEmpAttendance(att);

        const pr = await getApiData(() => getPayrollRecords().then(r => r.data), []);
        ...
        const lr = await getApiData(() => getLeaveRequests(undefined, mappedEmp.employeeId).then(r => r.data), []);
    ...
    return (
      <div className="p-4 lg:p-8 min-w-0 flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <Link href="/employees" className="mt-4 inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-700">
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 lg:p-8 min-w-0">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="mb-6">
        <Link href="/employees" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition mb-4">
          <ArrowLeft size={16} /> Back to Employees
        </Link>
      </motion.div>
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href={`/employees/register?id=${emp.id}`} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
              <Edit size={16} />
            </Link>
          </div>
        </div>
